...
const normalizeRule = (rule) =>
  ...
  */
  (0, punycode_1.toASCII)(rule).toLowerCase();
const parsePsl = (listContent) => {
  return {
    icann: extractByMarkers(
      listContent,
      config_1.PUBLIC_SUFFIX_MARKER_ICANN_START,
      config_1.PUBLIC_SUFFIX_MARKER_ICANN_END
    )
      .split(matchNewLine)
      .filter(containsRule)
      .map(normalizeRule),
    private: extractByMarkers(
      listContent,
      config_1.PUBLIC_SUFFIX_MARKER_PRIVATE_START,
      config_1.PUBLIC_SUFFIX_MARKER_PRIVATE_END
    )
      .split(matchNewLine)
      .filter(containsRule)
      .map(normalizeRule),
...
const buildTries = (psl, icannOverride) => {
  ...
  const icannTrie = (0, create_trie_1.createTrieFromList)([
    ...parsedPsl.icann,
    ...(icannOverride !== null && icannOverride !== void 0
      ? icannOverride
      : []),
  ]);
